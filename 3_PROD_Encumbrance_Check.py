...
response_filename = f"responses_prod_enc_{current_date}.json"
...
for retry in range(num_retries):
    for index, payload in enumerate(payloads, start=1):
        try:
            ...
            response = requests.post('http://scraper.advarisk.com:8090/encumbrance-search/', json=payload)
            time.sleep(0.015)
            # import pdb;pdb.set_trace()
...
excel_filename = str(custom_directory) + "responses_prod_enc_percent_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
