...
payload = {  # 22_1
        ...
        "search_attributes": {"district_code": "056",
                              "district_name": "उत्तरकाशी",
                              "tehsil_name": "पुरोला_*_पुरोला",
                              "tehsil_code": "00278_*_56785",
                              "village_code": "040101",
                              "village_name": "वेस्टी वल्ली",
                              "survey_no": "12",
                              "state_code": "UK"
                              },
...
state_ids = [{15:[994,1107,1162,1284,1009]}]

state_codes =[{"UK":"uk_satbara"}]
...
for index, (state_id, state_code) in enumerate(zip(state_ids, state_codes)):
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            query = f"""SELECT * FROM survey_meta AS sm LEFT JOIN village_meta AS v ON (v.village_id = sm.village_id) LEFT JOIN revenue_circle_meta AS rc ON (rc.rc_id = v.rc_id) LEFT JOIN taluka_meta AS t ON (t.taluka_id = rc.taluka_id) LEFT JOIN district_meta AS d ON (d.district_id = t.district_id) LEFT JOIN state_info AS s ON (s.state_id = d.state_id) WHERE s.state_id = {state_idx} and d.district_id = {district_idx} and sm.is_active = 1 and v.is_active = 1 ORDER BY RAND() LIMIT 1;"""
            ...
            if result:
                ...
                village_name_dev = result[0][11]
                tehsil_meta = result[0][38]
                tehsil_name_dev = result[0][35]
                pargana_meta = result[0][27]

                pargana_name_dev = result[0][24]
                district_code = result[0][47]
                district_name_dev = result[0][44]

                # Update payload with the fetched data
                payload["search_attributes"]["district_code"] = district_code
                payload["search_attributes"]["district_name"] = district_name_dev

                payload["search_attributes"]["tehsil_code"] = f"{tehsil_meta}_*_{pargana_meta}"
                payload["search_attributes"]["tehsil_name"] = f"{tehsil_name_dev}_*_{pargana_name_dev}"

                payload["search_attributes"]["village_code"] = village_meta
                payload["search_attributes"]["village_name"] = village_name_dev
...
for i, api_response in enumerate(callback_url_mapping):
    ...
    try:
        ...
        print(f"Target Scraper: {target_scraper}, Callback URL: {callback_url}, Task ID: {task_id}, Exception: {str(e)}",state_code, district_code,tehsil_code, village_code, survey_no)
...
excel_filename = str(custom_directory) + "responses_random_UK_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
