...
state_ids = [{14:[945,1010,1028,1047,1025]}]

state_codes =[{"JK":"jk_satbara"}]
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            if result:
                ...
                tehsil_name_eng = result[0][24]
                district_code = result[0][36]
                district_name_eng = result[0][33]

                # Update payload with the fetched data
                payload["search_attributes"]["district_code"] = district_code
                payload["search_attributes"]["district_name"] = district_name_eng

                payload["search_attributes"]["tehsil_code"] = tehsil_meta
                payload["search_attributes"]["tehsil_name"] = tehsil_name_eng

                payload["search_attributes"]["village_code"] = village_meta
                payload["search_attributes"]["village_name"] =village_name_eng
...
excel_filename = str(custom_directory) + "responses_random_JK_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
