...
for retry in range(num_retries):
    for index, payload in enumerate(payloads, start=1):
        try:
            ...
            response = requests.post('http://stage-scraper-api.advarisk.com:8090/create_task/', json=payload)
            time.sleep(0.015)
            # import pdb;pdb.set_trace()
...
ws.append(["Date", "Target Scraper", "Callback URL", "Task ID", "Status Message","Status", "State Code", "District Code","Tehsil Code", "Village Code", "Survey No","survey_code","Matching Percentage", "Matching Keys", "Unmatched Keys","Unmatched Data", "Exception"])

excel_file_path = "/Daily_Sanity/compare_file/Daily_Sanity_As_is.xlsx"
...
excel_filename = str(custom_directory) + "responses_stage_percent_Live_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
