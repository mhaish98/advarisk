...
#############################################################################
num_retries = 1;
state_ids = [{7:[661,829,652,639,656]}]

state_codes =[{"AP":"ap_webland"}]
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            if result:
                ...
                payload["search_attributes"]["survey_code"] = survey_no
...
excel_filename = str(custom_directory) + "responses_random_AP_web_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
