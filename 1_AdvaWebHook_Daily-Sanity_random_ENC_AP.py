...
state_ids = [{7:[1,2,3,4,5]}]

state_codes =[{"AP":"ap_encumbrance"}]
...
for index, (state_id, state_code) in enumerate(zip(state_ids, state_codes)):
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            random_number = random.randint(1, 60000000)
            while random_number in generated_numbers:
                random_number = random.randint(1, 60000000)
            generated_numbers.append(random_number)
            query = f"""SELECT * FROM property_prod.property_andhratransaction WHERE id ={random_number};"""
...
excel_filename = str(custom_directory) + "responses_random_AP_enc_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
