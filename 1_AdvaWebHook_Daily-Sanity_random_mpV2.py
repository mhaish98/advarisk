...
payload = {  # 26_3
        ...
        "search_attributes": {
            ...
            "survey_no": "5/6 (S)",
            "survey_code": "138070200037245002364"
        },
...
state_ids = [{2:[4,40,45,54,63]}]

state_codes =[{"MP":"mp_satbara_v2"}]
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            if result:
                ...
                village_name_eng = result[0][12]
                village_name_dev = result[0][11]
                tehsil_meta = result[0][27]
                tehsil_name_eng = result[0][25]
                tehsil_name_dev = result[0][24]
                district_code = result[0][36]
                district_name_eng = result[0][34]
                district_name_dev = result[0][33]

                # Update payload with the fetched data
                payload["search_attributes"]["district_code"] = district_code
                payload["search_attributes"]["district_name"] = f"{district_name_eng} | {district_name_dev}"

                payload["search_attributes"]["tehsil_code"] = tehsil_meta
                payload["search_attributes"]["tehsil_name"] = f"{tehsil_name_eng} | {tehsil_name_dev}"

                payload["search_attributes"]["village_code"] = village_meta
                payload["search_attributes"]["village_name"] =f"{village_name_eng} | {village_name_dev}"

                payload["search_attributes"]["survey_no"] = survey_no
                payload["search_attributes"]["survey_code"] = survey_code
...
excel_filename = str(custom_directory) + "responses_random_mpV2_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
