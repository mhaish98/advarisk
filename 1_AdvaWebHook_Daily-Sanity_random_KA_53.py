...
state_ids = [{9:[705,807,816,820,827]}]

state_codes =[{"KA":"ka_service53"}]
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            if result:
                ...
                payload["search_attributes"]["district_name"] = district_name
                payload["search_attributes"]["village_code"] = rc_meta +"_*_"+ village_meta
...
excel_filename = str(custom_directory) + "responses_random_KA_53_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
