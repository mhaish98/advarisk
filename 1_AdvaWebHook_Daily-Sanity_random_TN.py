...
#############################################################################
num_retries = 1;
state_ids = [{8:[665,682,691,701,897]}] # Replace with your state IDs

state_codes =[{"TN":"tn_satbara"}]
   ...
   # print(state_id)
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            query = f"""SELECT * FROM sub_survey_meta AS s LEFT JOIN survey_meta AS sm ON (sm.survey_id = s.survey_id) LEFT JOIN village_meta AS v ON (v.village_id = sm.village_id) LEFT JOIN taluka_meta AS t ON (t.taluka_id = v.taluka_id) LEFT JOIN district_meta AS d ON (d.district_id = t.district_id) LEFT JOIN state_info AS st ON (st.state_id = d.state_id) WHERE st.state_id =8 and d.district_id =665  and sm.is_active = 1 and v.is_active = 1 ORDER BY RAND() LIMIT 1;"""
            cursor.execute(query)
            result = cursor.fetchall()

            if result:
                # Assign the column values to variables
                sub_survey_no=result[0][1]
                survey_no = result[0][10]
                survey_code = result[0][12]
                village_meta = result[0][23]
                # village_name = result[0][11]
                tehsil_meta = result[0][36]
                # tehsil_name = result[0][24]
                district_code = result[0][45]
                ...
                payload["search_attributes"]["survey_no"] = survey_no+"#"+sub_survey_no
...
excel_filename = str(custom_directory) + "responses_random_TN_satbara_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
