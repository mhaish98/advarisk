...
state_ids = [{21:[952,999,1281,1170,1005]},{20:[1036,1108,1120,1125,1111]}]  # Replace with your state IDs

state_codes =[{"BH":"bh_satbara"},{"JH":"jh_satbara"}]
...
for index, (state_id, state_code) in enumerate(zip(state_ids, state_codes)):
    ...
    for i in list(state_id.values())[0]:
        ...
        try:
            ...
            query = f"""SELECT * FROM survey_meta AS sm LEFT JOIN khata_meta AS k ON (k.khata_id = sm.khata_id) LEFT JOIN village_meta AS v ON (v.village_id = k.village_id) LEFT JOIN revenue_circle_meta AS rc ON (rc.rc_id = v.rc_id) LEFT JOIN taluka_meta AS t ON (t.taluka_id = rc.taluka_id) LEFT JOIN district_meta AS d ON (d.district_id = t.district_id) LEFT JOIN state_info AS s ON (s.state_id = d.state_id) WHERE s.state_id = {state_idx} and d.district_id = {district_idx} and sm.is_active = 1 and v.is_active = 1 ORDER BY RAND() LIMIT 1;"""
            ...
            if result:
                ...
                account_no = result[0][11]
                village_meta = result[0][22]
                revenue_id = result[0][35]
                tehsil_meta = result[0][46]
                district_code = result[0][55]
                ...
                payload["search_attributes"]["revenue_circle_code"] = revenue_id
                payload["search_attributes"]["village_code"] = village_meta
                payload["search_attributes"]["account_no"] = account_no
...
excel_filename = str(custom_directory) + "responses_random_BH_JH_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx"
